...
if [ ! -d "/sdcard" ]; then
    err "Could not access storage — please allow permission and re-run"
fi
...
if [ -z "$SKIP_ENV" ]; then
    echo ""
    read -rp "  Output dir [/sdcard/Download]: " CUSTOM_OUT
    OUTPUT_DIR="${CUSTOM_OUT:-/sdcard/Download}"

    read -rp "  Agent port [9090]: " CUSTOM_PORT
    AGENT_PORT="${CUSTOM_PORT:-9090}"

    cat > "$ENV_FILE" << ENVEOF
# grab agent config — auto-generated by install.sh
...
ENVEOF
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-/sdcard/Download}"
